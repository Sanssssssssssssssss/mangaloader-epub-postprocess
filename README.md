# MangaLoader + EPUB Postprocess

`MangaLoader + EPUB Postprocess` is a lightweight Linux-first manga workflow for:
- downloading public manga chapter images
- packaging chapter folders into EPUB
- merging chapter EPUBs into one collected EPUB

The current target is Raspberry Pi 5 4GB and similar low-resource Linux environments.

## Acknowledgements

This project stands on top of excellent open-source work.

Special thanks to:
- Ciro Mattia Gonano and Pawel Jastrzebski for [Kindle Comic Converter (KCC)](https://github.com/ciromattia/kcc)
- lanyeeee for [copymanga-downloader](https://github.com/lanyeeee/copymanga-downloader)
- YuxuanHan0326 for [MangaEpubAutomation](https://github.com/YuxuanHan0326/MangaEpubAutomation)

Their work made it much faster to build a practical lightweight manga workflow.

## What This Project Is

This repo combines two practical pieces:
- a lightweight headless downloader
- a lightweight EPUB postprocess pipeline

There are currently two downloader tracks in the repo:
- `apps/copymanga-headless-rs`: the default Rust headless CLI that directly reuses the upstream download core
- `apps/copymanga-headless-lite`: the earlier Python prototype, kept only as a legacy reference

The main product entry lives here:
- `apps/manga-pipeline-lite`

It is intentionally:
- headless
- CLI-first
- PowerShell-free
- GUI-free
- upscaling-free

The default downloader backend is now the Rust upstream-core path. `install-pi.sh` builds a repo-local downloader binary under `.tools/manga-pipeline-lite/bin/copymanga-headless-rs`.

## Quick Start

### Raspberry Pi

```bash
git clone https://github.com/Sanssssssssssssssss/mangaloader-epub-postprocess.git
cd mangaloader-epub-postprocess
chmod +x apps/manga-pipeline-lite/install-pi.sh
chmod +x apps/manga-pipeline-lite/bin/manga-pipeline-lite
./apps/manga-pipeline-lite/install-pi.sh
./apps/manga-pipeline-lite/bin/manga-pipeline-lite doctor --config apps/manga-pipeline-lite/config.pi5.json
./apps/manga-pipeline-lite/bin/manga-pipeline-lite run --config apps/manga-pipeline-lite/config.pi5.json
```

### Main Commands

```bash
./apps/manga-pipeline-lite/bin/manga-pipeline-lite init-config --output my-job.json
./apps/manga-pipeline-lite/bin/manga-pipeline-lite doctor --config my-job.json
./apps/manga-pipeline-lite/bin/manga-pipeline-lite download --config my-job.json
./apps/manga-pipeline-lite/bin/manga-pipeline-lite package --config my-job.json
./apps/manga-pipeline-lite/bin/manga-pipeline-lite merge --config my-job.json
./apps/manga-pipeline-lite/bin/manga-pipeline-lite run --config my-job.json
```

## How To Use

1. Install the Pi toolchain with `install-pi.sh`.
2. Copy `apps/manga-pipeline-lite/config.pi5.json` or `config.example.json`.
3. Edit these fields:
   - `downloader_cmd`
   - `comic_path_word`
   - `group_path_word`
   - `chapter_limit`
   - `author`
   - `title`
   - optional downloader pacing fields such as `chapter_workers`, `image_workers`, `api_retries`, `chapter_interval_sec`, and `image_interval_sec`
4. Run `doctor` first to confirm dependencies and metadata access.
5. Run `run` for the full pipeline, or use `download/package/merge` separately.

### Complete Example Config

General example:

```json
{
  "job_name": "fire-force-public-smoke",
  "job_root": "",
  "downloader_cmd": "./.tools/manga-pipeline-lite/bin/copymanga-headless-rs",
  "comic_path_word": "yanyanzhixiaofangdui",
  "group_path_word": "default",
  "chapter_limit": 1,
  "author": "Atsushi Ohkubo",
  "title": "Fire Force Vol 1",
  "kcc_cmd": "/tmp/kcc-venv/bin/kcc-c2e",
  "profile": "KS",
  "language": "zh",
  "description": "Generated by manga-pipeline-lite",
  "contributor": "manga-pipeline-lite",
  "chapter_workers": 1,
  "image_workers": 4,
  "api_retries": 5,
  "retry_base_sec": 1.0,
  "retry_jitter_sec": 0.5,
  "risk_wait_sec": 60.0,
  "chapter_interval_sec": 0.0,
  "image_interval_sec": 0.0,
  "skip_existing": true,
  "max_images_per_chapter": 3,
  "steps": {
    "download": true,
    "package": true,
    "merge": true
  }
}
```

Raspberry Pi oriented example:

```json
{
  "job_name": "pi5-public-job",
  "job_root": "",
  "downloader_cmd": "./.tools/manga-pipeline-lite/bin/copymanga-headless-rs",
  "comic_path_word": "yanyanzhixiaofangdui",
  "group_path_word": "default",
  "chapter_limit": 1,
  "author": "Atsushi Ohkubo",
  "title": "Fire Force Pi Edition",
  "kcc_cmd": "./.tools/manga-pipeline-lite/bin/kcc-c2e",
  "profile": "KS",
  "language": "zh",
  "description": "Generated by manga-pipeline-lite on Raspberry Pi 5 4GB",
  "contributor": "manga-pipeline-lite-pi",
  "chapter_workers": 1,
  "image_workers": 2,
  "api_retries": 5,
  "retry_base_sec": 1.0,
  "retry_jitter_sec": 0.5,
  "risk_wait_sec": 60.0,
  "chapter_interval_sec": 1.0,
  "image_interval_sec": 0.0,
  "skip_existing": true,
  "max_images_per_chapter": null,
  "steps": {
    "download": true,
    "package": true,
    "merge": true
  }
}
```

Downloader stability fields:
- `downloader_cmd`: repo-local Rust downloader binary path, or another compatible command
- `chapter_workers`: chapter-level downloader concurrency
- `api_retries`: API and image request retry count
- `retry_base_sec`: base wait before retrying
- `retry_jitter_sec`: random extra wait added to retries
- `risk_wait_sec`: wait time after upstream risk-control responses
- `chapter_interval_sec`: sleep between chapters
- `image_interval_sec`: sleep after each image download

Each job writes outputs to:

```text
runs/<job-name>/
  downloads/
  epubs/
  merged/
  config.lock.json
  result.json
```

## Folder Guide

These folders are the ones you will most often see:

- `apps/`
  - runnable product code
  - `apps/copymanga-headless-rs`: default upstream-core-aligned Rust headless downloader
  - `apps/copymanga-headless-lite`: legacy Python downloader prototype
  - `apps/manga-pipeline-lite`: main product entry, Pi delivery scripts, and EPUB postprocess helpers
- `skills/`
  - reusable skill docs for the lightweight packaging workflow

These folders are local/runtime-oriented and may exist only after setup or testing:

- `.tools/`
  - local install artifacts such as the repo-local KCC wrapper
  - ignored by git
- `runs/`
  - output folders created by real jobs and smoke tests
  - ignored by git
- `imports/`
  - local copies of upstream repositories used during evaluation
  - ignored by git

## License

This repository is released under the MIT License. See [LICENSE](LICENSE).
