...
- package chapter folders with `scripts/manga_packager.py`
- generate merge plans with `scripts/make_merge_plan.py`
- merge chapter EPUBs with `scripts/merge_epub_by_order.py`
...
- use `scripts/manga_packager.py` for package-only
- use `scripts/manga_packager.py` with `--merge-output` for package-plus-merge
- use `scripts/make_merge_plan.py` plus `scripts/merge_epub_by_order.py` for merge-only
...
- use `scripts/make_merge_plan.py` to build the plan JSON
...
Use `scripts/merge_epub_by_order.py --plan <plan.json>` to create the final anthology EPUB.
...
- `scripts/manga_packager.py`: Linux-native package-only or package-plus-merge entry point
- `scripts/make_merge_plan.py`: generate merge-plan JSON from an EPUB directory
- `scripts/merge_epub_by_order.py`: merge ordered chapter EPUBs into one anthology EPUB
